...
public final class Parser extends DefaultHandler
{
        ...
        try
        {
            ...
            else if (qName.equals("OBJModel"))
            {
             ...
             // parse name attribute
                ...
                if (name == null) throw new ParseException("Element \"" + qName + "\" requires attribute \"model\".");

                // call handler
                if (echoHandler != null) echoHandler.startOBJModel(size, name, model);
                if (    handler != null) handler.startOBJModel(size, name, model);
            }
            ...
            else if (qName.equals("Scene"))
            {
             ...
             // parse antialiasing attribute
                String antialiasingString = attributes.getValue("antialiasing");
                if (antialiasingString == null) throw new ParseException("Element \"" + qName + "\" requires attribute \"antialiasing\".");
                Boolean antialiasing = Boolean.valueOf(antialiasingString);

                // call handler
                if (echoHandler != null) echoHandler.startScene(camera, lights, background, antialiasing);
                if (    handler != null) handler.startScene(camera, lights, background, antialiasing);
            }
            // Shape element
            else if (qName.equals("Shape"))
            {
                ...
                // parse color reflection
                String reflectionColorString = attributes.getValue("reflection");
                if (reflectionColorString == null) throw new ParseException("Element \"" + qName + "\" requires attribute \"reflection\".");
                Color3f reflection = ParserUtils.parseColor3f(reflectionColorString);
                
             // parse weight1 attribute
                String refractionString = attributes.getValue("refraction");
                if (refractionString == null) throw new ParseException("Element \"" + qName + "\" requires attribute \"refraction\".");
                float refraction = ParserUtils.parseFloat(refractionString);

                // call handler
                if (echoHandler != null) echoHandler.startShape(geometry, material, texture, reflection, refraction);
                if (    handler != null) handler.startShape(geometry, material, texture, reflection, refraction);
            }
            // Rotate element
    ...
    public void endElement(String uri, String localName, String qName) throws SAXException
    {
        try
        {
            ...
            {
                // call handler
                if (echoHandler != null) echoHandler.endOBJModel();
                if (    handler != null) handler.endOBJModel();
            }
            // IndexedTriangleSet element
            else if (qName.equals("IndexedTriangleSet"))
            {
